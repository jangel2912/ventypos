var classVentaOffline = function () {
  ...
  this.queryProductosNavegador = function (callback) {
    ...
    db.transaction(
      function (tx) {
        var sql =
          'SELECT "vendtyShop.jpg" AS imagen, producto.nombre, producto.codigo, precio_compra, precio_venta, stock_actual.unidades AS stock_minimo, IFNULL(impuesto.porciento, 0) AS impuesto, "vendtyShop.jpg" AS imagen, producto.id FROM producto LEFT JOIN stock_actual ON producto.id = stock_actual.producto_id INNER JOIN usuario_almacen ON usuario_almacen.almacen_id = stock_actual.almacen_id LEFT JOIN impuesto ON impuesto.id_impuesto = producto.impuesto WHERE  producto.material=0 AND usuario_almacen.usuario_id = ' +
          id_user +
  ...
  this.queryProductosBuscador = function (query, callback) {
    ...
    if (query == "") {
      ...
      db.transaction(
        function (tx) {
          var sql =
            "SELECT producto.codigo_barra, producto.nombre, producto.codigo, precio_compra, ubicacion AS ubic, precio_venta, stock_actual.unidades AS stock_minimo, IFNULL(impuesto.porciento, 0) AS impuesto, 'vendtyShop.jpg' AS imagen, producto.id FROM producto INNER JOIN categoria ON categoria.id = producto.categoria_id LEFT JOIN stock_actual ON producto.id = stock_actual.producto_id INNER JOIN usuario_almacen ON usuario_almacen.almacen_id = stock_actual.almacen_id LEFT JOIN impuesto ON impuesto.id_impuesto = producto.impuesto WHERE  producto.material=0 AND (UPPER(producto.nombre) LIKE '%" +
            query +
  ...
  this.queryProductosCodificalo = function (query, callback) {
    ...
    if (query == "") {
      ...
      db.transaction(
        function (tx) {
          var sql =
            " SELECT 'vendtyShop.jpg' AS imagen, producto.nombre, producto.codigo, precio_compra, ubicacion AS ubic, precio_venta, stock_actual.unidades AS stock_minimo, IFNULL(impuesto.porciento, 0) AS impuesto, 'vendtyShop.jpg' AS imagen, producto.id FROM producto LEFT JOIN stock_actual ON producto.id = stock_actual.producto_id INNER JOIN usuario_almacen ON usuario_almacen.almacen_id = stock_actual.almacen_id INNER JOIN categoria ON producto.categoria_id = categoria.id LEFT JOIN impuesto ON impuesto.id_impuesto = producto.impuesto WHERE producto.material=0 AND producto.codigo = '" +
            query +
            "' AND usuario_almacen.usuario_id = '" +
            id_user +
            "' AND producto.activo = 1 ";
  ...
  this.queryFactura = function (id, callback) {
    ...
    db.transaction(
      ...
      function (tx, res) {
        ...
        objFactura = data;
        console.log("en appVentasoffline:");
        console.log(data);
        callback();
      }
    );
