...
$(document).ready(function(){
	...
	/*	if(row.ubic != '' && row.impuesto != ''){	
	 ...
	 if($sinprecio == 'si'){
        ...
        rowHtml += "<td><input type='hidden' class='precio-prod-real' value='"+$('.precio-real').eq($(this).index()).val()+"'/><input type='hidden' class='precio-prod-real-no-cambio' value='"+$('.precio-real').eq($(this).index()).val()+"'/></td>";
        ...
        $("#pagar").click(function(){
            ...
            $("#valor_pagar_hidden").val(Math.round(parseInt($("#total").val())));
               ...
               if(data != null){
                   ...
                   var precioimpuesto =   (parseInt(sProduct.precio_venta) * parseInt(sProduct.impuesto) / 100 +parseInt(sProduct.precio_venta) );
					...
					if(sProduct.impuesto != '0'){
					 $('#cod-precio-impuesto').html("<strong>Precio + IVA:</strong> "+formatDollar(precioimpuesto)+"<br>");
					}
					if(sProduct.impuesto == '0'){
					 $('#cod-precio-impuesto').html("<strong>Precio de venta:</strong> "+formatDollar(precioimpuesto)+"<br>");	
					}					
                ...
                $('#vitrina').append(
                   '<div class="vitrina-item" onclick="categoria_producto('+i+')">'+    
                        ...
                        '<div id="pie-item">'+
                            ...
                            '<div align="center" style="background: #5327AF;" ><h5 >$'+response[i].precio_venta+'</h5></div>'+
                        '</div>'+
                    '</div>'
