<div class="page-header">
    <div class="icon">
     ...
     <h1 class="sub-title"><?php echo custom_lang("Ventas Online", "Ordenes"); ?></h1>
</div>
...
<div class="row-fluid">
    <div class="span12">
        <ul class="nav nav-tabs" role="tablist">
            <li role="presentation" id="video1" class="active"><a href="#info" aria-controls="home" role="tab" data-toggle="tab" style="padding-right: 40px; padding-left: 40px;">Ordenes</a></li>
            <li role="presentation"><a id="video" href="#img" aria-controls="profile" role="tab" data-toggle="tab" style="padding-right: 40px; padding-left: 40px;">Facturas</a></li>
            <li role="presentation"><a id="video" href="#img" aria-controls="profile" role="tab" data-toggle="tab" style="padding-right: 40px; padding-left: 40px;">Comandas</a></li>
            <li role="presentation"><a id="video" href="#img" aria-controls="profile" role="tab" data-toggle="tab" style="padding-right: 40px; padding-left: 40px;">Domicilios</a></li>
        </ul>
    </div>
    <div class="span12" style="margin-left: 0.864102564102564%;">
        <div class="block">
            <div class="head blue">
                <!-- <h2><?php echo custom_lang('sima_outstanding_all', "Listado de Ordenes"); ?></h2> -->
            </div>
                ...
                <table class="table aTable" cellpadding="0" cellspacing="0" width="100%" id="ventas_onlineTable">
                    <thead>
                        <tr>
                            ...
                            <th width="10%"><?php echo custom_lang('ventaonline_customer_dni', "Teléfono"); ?></th>
                            <th width="12%"><?php echo custom_lang('ventaonline_valor', "Monto"); ?></th>
                            <th width="15%"><?php echo custom_lang('ventaonline_email', "Origen"); ?></th>
                            <th width="13%"><?php echo custom_lang('ventaonline_estado', "Estado"); ?></th>
                            <th ><?php echo custom_lang('ventaonline_estado', "Pago"); ?></th>
                            <th ><?php echo custom_lang('ventaonline_entrega', "Entrega"); ?></th>
...
<div class="modal" id="almacenModal" tabindex="-1" role="dialog"  aria-hidden="true">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="myModalLabel">&nbsp; Cambiar de Almacen la Orden #<span id="atend_id"></span>  </h4>
            </div>
            <div class="modal-body">
                <div id="atend_fecha" class="span" style="text-align: right; position: absolute; margin-left: 0px !important"></div>
                <div class="span6" style="margin-left: 0px !important"> &nbsp; </div>
                ...
                <div class="span6">
                    ...
                    <center>
                        <span id="atend_msg"></span>
                        <br>
                        <input type="hidden" id="c_almacen_id" name="c_almacen_id" value=""/>
                        <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
                        <button type="submit" class="btn btn-success" id="cambiarAlmacen">Confirmar</button>
                    </center>
                    <br>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $(document).on('click','#cambiarAlmacen',function(event) {
            $.post ("<?php echo site_url("ventas_online/cambiarAlmacen") ?>",
                {
                    'id':$('#almacenModal').find('#c_almacen_id').val(),
                    'almacen_id':$('#almacenModal').find('#almacen option:selected').val(),
                },
                function(data) {
                    if (data.json == 1) {
                        alert("Se ha cambiado la orden de almacen");
                        location.reload();
                    }
                },'json'
            )
        ...
        $(document).on('click','#comandarOrden',function(event) {
            $.post ("<?php echo site_url("ventas_online/comandarOrden") ?>",
                {
                    'id': $(this).data('id'),
                },
                function(data) {
                    if (data) {
                        alert("Su orden ha sido comandada en la APP Vendty Comanda Virtual");
                    } else {
                        alert("Error al intentar comnadar");
                    }
                },'json'
            )
        });
        ...
        $('#ventas_onlineTable').dataTable({
            ...
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo site_url("ventas_online/get_ajax_data_orden"); ?>",
            ...
            "aoColumnDefs": [
                {
                    ...
                    "mRender": function (data, type, row) {
                        if(row[12] != null && row[13] != null){
                            ...
                            return "<small class='label label-warning'>Inmediata</small>";
                        }
                    }
                },
                {"bSortable": false, "aTargets": [9], "bSearchable": false,
                    "mRender": function (data, type, row) {
                        ...
                        <?php if (in_array('57', $permisos) || $is_admin == 't'){ ?>
                            buttons += '<a style="cursor:pointer;"  id="d_' + row[0] + '" data-toggle="modal" data-target="#myModal"  class="button default detall acciones" data-tooltip="Detalles" title="Ver Detalles"><div class="icon"><img alt="editar" data-cambiar="<?php echo $this->session->userdata('new_imagenes')['editar']['cambio'] ?>" data-original="<?php echo $this->session->userdata('new_imagenes')['editar']['original'] ?>" class="iconacciones" src="<?php echo $this->session->userdata('new_imagenes')['editar']['original'] ?>"></div></a>';
                        <?php } ?>
                            if (row[6] == 'Atendida') {
                            <?php if (in_array('11', $permisos) || $is_admin == 't'){ ?>
                                        buttons += '<a href="" id="a_' + row[0] + '" data-toggle="modal" data-target="#almacenModal" data-tooltip="Cambiar almacén" class="button default facturar acciones" title="Facturar"><div class="icon"><img alt="Facturar" data-cambiar="<?php echo $this->session->userdata('new_imagenes')['tiendaStatus']['cambio'] ?>" data-original="<?php echo $this->session->userdata('new_imagenes')['tiendaStatus']['original'] ?>" class="iconacciones" src="<?php echo $this->session->userdata('new_imagenes')['tiendaStatus']['original'] ?>"></div></a>';
                                        buttons += '<a href="" id="a_' + row[0] + '" data-toggle="modal" data-target="#facturarModal" data-tooltip="Facturar" class="button default facturar acciones" title="Cambiar de Almacen"><div class="icon"><img alt="Cambiar de Almacen" data-cambiar="<?php echo $this->session->userdata('new_imagenes')['facturar']['cambio'] ?>" data-original="<?php echo $this->session->userdata('new_imagenes')['facturar']['original'] ?>" class="iconacciones" src="<?php echo $this->session->userdata('new_imagenes')['facturar']['original'] ?>"></div></a>';
                                        buttons += '<a href="javascript:void(0);" id="comandarOrden" data-id="' + row[0] + '" data-tooltip="Comandar" class="button default acciones" title="Comandar"><div class="icon"><img alt="Comandar" data-cambiar="<?php echo $this->session->userdata('new_imagenes')['ordenanofectada']['cambio'] ?>" data-original="<?php echo $this->session->userdata('new_imagenes')['ordenanofectada']['original'] ?>" class="iconacciones" src="<?php echo $this->session->userdata('new_imagenes')['ordenanofectada']['original'] ?>"></div></a>';
                            <?php } ?>
                            }else if (row[6] =='Pendiente pago' || row[6]=='Aprobada' || row[6] == 'Facturada'){
                                buttons += '<a href="<?php echo site_url('ventas/imprimir/') ?>/'+row[11]+'/copia" style="cursor:pointer;"  id="a_' + row[0] + '" data-tooltip="Imprimir" class="button default btn-print confirmar acciones" title="Ver factura" target="_blank"><div class="icon"><img alt="imprimir" data-cambiar="<?php echo $this->session->userdata('new_imagenes')['imprimir']['cambio'] ?>" data-original="<?php echo $this->session->userdata('new_imagenes')['imprimir']['original'] ?>" class="iconacciones" src="<?php echo $this->session->userdata('new_imagenes')['imprimir']['original'] ?>" ></div></a>';
                            }

                            buttons += '<a href="" id="' + row[0] + '" data-tooltip="Eliminar" class="button red anular acciones" title="Anular"><div class="icon"><img alt="Eliminar" data-tooltip="Rechazar Orden" data-cambiar="<?php echo $this->session->userdata('new_imagenes')['eliminar']['cambio'] ?>" data-original="<?php echo $this->session->userdata('new_imagenes')['eliminar']['original'] ?>" class="iconacciones" src="<?php echo $this->session->userdata('new_imagenes')['eliminar']['original'] ?>"></div></a>';
                        buttons += "</div>";
                        return buttons;
                    }
                }
        ...
        $('body').on('click', '.detall', function (e) {
            ...
            $.getJSON( url + '/' + id[1],
                function (data) {
                    ...
                    if(data.schedule != ""){
                        notas_html+='<b>Entrega:</b> ' + data.schedule + '<br>';
                    }
                    else {
                        notas_html+='<b>Entrega:</b> <small class="label label-warning">Inmediata</small><br>';
                    }

                    if(data.notas_adicionales != ""){
                        notas_html+='<b>Notas adicionales:</b> ' + data.notas_adicionales + '<br>';
                    }
            ...
            $.getJSON(
                ...
                function (data) {
                    if(data.estado == 0) {
                        ...
                        $('#facturarModal').find('#atend_nombre').html("Nombre: " + data.nombre);
        ...
        $('body').on('click', '.anular',
            function (e) {
                e.preventDefault();
                var url = "<?php echo site_url('ventas_online/eliminar_solicitud_ventas_orden/') ?>";
