<script type="text/javascript"> var client = <?php echo json_encode($data['clientes']) //2015-12-17 ?></script>
<script src="<?php echo base_url("index.php/OpcionesController/index")?>"></script>
<script type="text/javascript">allert(11);
...
<!-- 
                    ...
                    <div class="row-form" style="padding-left: 0px;padding-right: 0px; padding-top:4px;">
                        <form>
                            <ul id='tipo-busqueda'>
                                <li id ='buscalo' <?php if($_REQUEST['var'] == "buscalo"){ echo "class='active'"; } ?> > <h3><img src="<?php echo base_url("/public/img/");?>/buscador.png" width="45px" height="15px" />&nbsp;&nbsp; BUSCADOR </h3><!--  <img src="<?php //echo base_url("/public/img/");?>/codigobarra-icon.png">  --></li>
                                <li id ='codificalo' <?php if($_REQUEST['var'] == "codificalo"){ echo "class='active'"; } ?> > <h3><img src="<?php echo base_url("/public/img/");?>/codigo_barra.png" width="40px" height="15px" />&nbsp;&nbsp; LECTOR </h3> </li>
                                <li id ='navegador'  <?php if($_REQUEST['var'] == "navegador"){ echo "class='active'"; } ?> > <h3><img src="<?php echo base_url("/public/img/");?>/navegador.png" width="40px" height="15px" />&nbsp;NAVEGADOR</h3>  </li>
                            </ul>
                        </form>
...
<table height="14px"><tbody><tr><td></td></tr></tbody></table>       
     ...
     <?php if($data['comanda']=='si'){ ?>
		...
		<?php if( $data['nit']=='320001127839dcf' 
        || $this->session->userdata('base_dato') == 'vendty2_db_1542_venta2015'
        || $this->session->userdata('base_dato') == 'vendty2_db_5550ee88dc62b'  
        || $this->session->userdata('base_dato') == 'vendty2_db_1150_image2015_09_21'
        || $this->session->userdata('base_dato') == 'vendty2_db_54cb1c75472a1'
        || $this->session->userdata('base_dato') == 'vendty2_db_2286_dcfst2016'
        || $this->session->userdata('base_dato') == 'vendty2_db_2476_Workt2016'
        || $this->session->userdata('base_dato') == 'vendty2_db_1932_pipef2016')
        { ?>
                
                <button class="btn btn-primary btn-large" id="planSepare">
                    Plan Separe
                </button> 	
		<?php 
        } 
...
<table>
<tr>
<td style=" width:37%">&nbsp;</td><td style="width:210px;">  
                            <button style="border: 0; vertical-align: middle; height:28px; padding: 4px 8px;" type="button"   id="pendiente"  onclick="generar_boton()"   class="btn btn-primary btn-large" title="Esperar"><span class="ico-plus" style=" margin: 4px 8px 0px 0px;"></span>Espera</button></td>
							<td style="width:210px;">  
                            <button style="border: 0; vertical-align: middle; width:150px; height:28px; padding: 4px 8px;" type="button"   id="actualizar_pendiente"  onclick="generar_boton()"   class="btn btn-primary btn-large" title="Esperar"><span  style=" margin: 4px 8px 0px 0px;"></span>Actualizar V en Espera</button></td>
</td>
</tr>
</table>
  </div>
...
<div id="dialog-sobrecosto-form"  title="<?php echo custom_lang('sima_pay_information', "Sobrecosto (opcional)");?>">
        ...
        <form id="client-form">
            <div class="row-form" class="data-fluid">
                ...
                <div class="span4">
                    <?php 
                          if($this->session->userdata('base_dato') == 'vendty2_db_5617e855ce4ad') { ?>
                            <input type="number" name="sobrecostos_input" id="sobrecostos_input" value="0" />
                    <?php } else { ?>
...
<input type="hidden" name="valor_entregado" id="valor_entregado5_plan" value="0" />
 ...
 <div align="center"> 
                       ...
                       <input type="button" value="Cancelar"  id="cancelar_plan" class="btn btn-warning"/> 
  </div>
        </form>
...
<?php if($data['multiples_formas_pago']!='si'){ ?>
                ...
                <div class="row-form">
                    ...
                    <div class="span3">
                        <input class="dataMoneda1" type="number" name="valor_entregado" id="valor_entregado"/>
        ...
        $(".car"+data[i].id+"").live('touchstart dblclick', function(){
		      var valor = $(this).attr('id');
			           ...
			           $.ajax({
                            ...
                            data: {  id: 1 },
                            success: function(data) {
						      for(var i in data){   if(data[i].id !== '-1' && data[i].usuario_id == <?php echo $this->session->userdata('user_id'); ?> && data[i].id != valor){ 
...
---------------------------------------------------*/	
 ...
 function cambiar_color(id){
                            ...
                            success: function(data) {
                              for(var i in data){ 
                                 ...
                                 // document.getElementById(''+data[i].id+'').style.backgroundColor = '#DD00DD';
								  $('#'+data[i].id+'').attr('style', 'width: 79px; padding: 1px 1px; height: 45px; vertical-align: text-bottom;background-color: #5E8C47 !important');
                                 }
                              } 
       ...
       // document.getElementById(''+id+'').style.backgroundColor = '#DD00DD';
		$('#'+id+'').attr('style', 'width: 79px; padding: 1px 1px; height: 45px; vertical-align: text-bottom;background-color: blue !important');
}
...
function renderFactura(){
	...
	<?php  if(in_array("1010", $permisos) && $is_admin !== 't'){ ?>
     ...
     <?php
	   ...
	   ?>
        rowHtml += "<td><span class='label label-success precio-prod'  onClick='calculadora_descuento("+sProduct.precio_venta+");'>"+formatDollar(sProduct.precio_venta)+"</span><input type='hidden' class='precio-prod-precio-porcentaje' /><input type='hidden' class='precio-prod-real' value='"+sProduct.precio_venta+"'/><input type='hidden' class='precio-prod-real-no-cambio' value='"+sProduct.precio_venta+"'/></td>";
     <?php }  ?>			
          ...
          $.ajax({
                url: "<?php echo site_url("puntos/get_por_compras_puntos")?>/"+$("#total").val(),
                 success: function(response) {	 
				     if(response == 'si'){ document.getElementById('asignar_plan').style.display = 'block';  }
					 else{ document.getElementById('asignar_plan').style.display = 'none';   }
...
$("#pagar").click(function(){
 ...
 (parseInt($("#valor_entregado").val()) + parseInt($("#valor_entregado1").val()) + parseInt($("#valor_entregado2").val())  
	+ parseInt($("#valor_entregado3").val())  + parseInt($("#valor_entregado4").val()) + parseInt($("#valor_entregado5").val()))   - $("#valor_pagar_hidden").val()
				  ...
				  if(forma_pago_id == 'forma_pago1'){ $("#valor_entregado1").val(0); $("#valor_entregado1").removeAttr("disabled"); }
				  if(forma_pago_id == 'forma_pago2'){ $("#valor_entregado2").val(0); $("#valor_entregado2").removeAttr("disabled"); }
				  if(forma_pago_id == 'forma_pago3'){ $("#valor_entregado3").val(0); $("#valor_entregado3").removeAttr("disabled"); }
				  if(forma_pago_id == 'forma_pago4'){ $("#valor_entregado4").val(0); $("#valor_entregado4").removeAttr("disabled"); }
				  if(forma_pago_id == 'forma_pago5'){ $("#valor_entregado5").val(0); $("#valor_entregado5").removeAttr("disabled"); }
...
//puntos -------------------------------------------------------------------
    ...
    function load_provincias_from_pais(pais){
	   ...
	   $.ajax({
                          ...
                          if(pais != 'Colombia'){  $("#provincia").html(''); }
	...
	function plan(){
     ...
     if($('#plan_puntos').prop('checked') == true){
		...
		else {  document.getElementById('escoger_plan').style.display = 'none';    }
