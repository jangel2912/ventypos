...
<?php 
	$id_licencia=$data['empresa'][0]->id_licencia;
	
	?>
    <h1><?php echo custom_lang("PagosLicencias", "Pagos de Licencia $id_licencia ");?></h1>
...
<div class="row-fluid">
    <div class="span12">
        <div class="block">
			...
			<?php endif; 				
			?>
            <a href="<?php echo site_url("administracion_vendty/pagos_factura/nuevo/$id_licencia")?>" class="btn btn-success"><small class="ico-plus icon-white"></small><?php echo custom_lang('', "Nuevo Pago");?></a>
  
			            			
            <div class="head blue">
                <div class="icon"><i class="ico-box"></i></div>
                <h2><?php echo custom_lang('', "Todas los Pagos Licencia $id_licencia");?></h2>
            </div>
						...
						<table class="table" cellpadding="0" cellspacing="0" width="100%" id="pagos">
							...
							<tbody>
										...
										echo'<tr>';
											...
											if(isset($data['facturas'])){
												...
												foreach ($data['facturas'] as $key2 => $value2) {
													if($value2['id_factura_licencia']==$value['id_factura_licencia']){
								...
								<!--	<a href="<?php echo site_url('administracion_vendty/pagos_factura/editar/'.$value['idpagos_licencias']);?>" class="button default"  data-tooltip="Editar"><div><span class="glyphicon glyphicon-pencil"></span></div></a>												
									...
									<a href="<?php echo site_url('administracion_vendty/facturas_licencia/regenerar_factura_electronica/'.$id_licencia.'/'.$value['idpagos_licencias']);?>" onclick="if(confirm('Esta seguro que desea generar la factura electronica de la licencia')){return true;}else{return false;}" class="button default"  data-tooltip="Generar Factura Electronica"><div><span class="glyphicon glyphicon-file"></span></div></a>
									<?php } ?>
								<?php	
											echo'</td>';										
										echo'</tr>';
