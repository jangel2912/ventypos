...

{
	...
	function index($offset = 0){
        ...
        */
        ...
        $this->layout->template('member')
            ...
            ->show('credits/index', array('data' => $data, 'datacurrency' => $datacurrency));

        /*  ANTERIOR VERSION DE CREDITOS
            ...
            $this->layout->template('member')
                ->css(array(base_url("/public/css/ventas.css"), base_url("/public/fancybox/jquery.fancybox.css")))
                ->js(array(base_url("/public/js/ventas.js"), base_url("/public/fancybox/jquery.fancybox.js")))
        	->show('credito/index_pendientes', array('data' => $data));*/
        }
		...
		redirect('auth', 'refresh');
            ...
            $this->layout->template('member')
                ->css(array(base_url("/public/css/ventas.css"), base_url("/public/fancybox/jquery.fancybox.css")))
                ->js(array(base_url("/public/js/ventas.js"), base_url("/public/fancybox/jquery.fancybox.js")))
		->show('credito/index_pagadas',array('data'=>$data));
	}


        public function get_ajax_data(){
            
            $this->output->set_content_type('application/json')->set_output(json_encode($this->facturas->get_ajax_data()));

        }

        

        public function get_ajax_data_pagadas(){
        ...
        public function get_ajax_data_pendientes(){
    ...
    public function imprimir($id, $offset = 0){
        ...
        {
            $this->layout->template('ajax')->show('credito/_imprime', array('data' => $data)); 
        }else
        {
            $this->layout->template('ajax')->show('credito/_imprimeTirilla', array('data' => $data)); 
        }
    }

	

	function imprimir_ant($id=0)
...
EOD;
            ...
            $pdf_name = "Factura {$data_factura["numero"]}.pdf";
...
EOF;
        ...
        public function excel_data_informe() {
            
            $informe = $this->creditos->get_ajax_data();
            $reporte = $this->load->library('phpexcel');
    
            $reporte = new PHPExcel();
            $reporte->setActiveSheetIndex(0);
    
            $reporte->getActiveSheet()->setCellValue('A1', 'Número');
            $reporte->getActiveSheet()->setCellValue('B1', 'Cliente');
            $reporte->getActiveSheet()->setCellValue('C1', 'Precio total');
            $reporte->getActiveSheet()->setCellValue('D1', 'Retenciones');
            $reporte->getActiveSheet()->setCellValue('E1', 'Saldo');
            $reporte->getActiveSheet()->setCellValue('F1', 'Fecha');
            $row = 2;
            ...
            header('Content-Type: application/vnd.ms-excel');
            header('Content-Disposition: attachment;filename="cuentas por cobrar.xlsx"');
            header('Cache-Control: max-age=0');
            header('Cache-Control: max-age=1');
        ...
        public function credits(){
            $this->layout->template('member')->show('credits/index');
        }
