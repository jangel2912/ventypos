...
class Inventario extends CI_Controller {
    ...
    public function index() {
        ...
        $this->layout->template('member')
                ...
                ->show('inventario/index',array("data" => $data));
    }
    ...
    public function nuevo() {
        ...
        if (isset($_POST['tipo_movimiento'])) {
            //verificamos si la existencia existe en el almacen para el caso de traslados
            if($_POST['tipo_movimiento'] == 'traslado') {
                //verificamos la disponibilidad por producto en el almacen seleccionado
                $productos  = $_POST['productos'];
                //print_r($productos);
                foreach($productos as $producto){
                    $stock_actual = $this->stock->get_by_prod_almac($_POST['almacen_id'],$producto['producto_id']);
                    if($producto['cantidad'] > $stock_actual['unidades'])
                   {
                      //return $this->output->set_content_type('application/json')->set_output(json_encode(array('success' => false,'mensaje' => 'La cantidad disponible en almacen es inferior a la del traslado')));
                        return $this->output->set_content_type('application/json')->set_output(json_encode(array('success' => false,'mensaje' => 'La cantidad disponible en almacen es inferior a la del traslado')));
                   }
            ...
            //verificamos si tiene activo el costo promedio
            /*if($costo_promedio == 1){
                die("Asdsd");
            }*/
            ...
            $data['almacenes'] = $this->almacenes->get_combo_data(null,false);
            //$id_almacenes = array_keys($data['almacenes']);
            //print_R($this->session->userdata('licencia_admin_vencida'));
            if($this->session->userdata('licencia_admin_vencida') != null && $this->session->userdata('licencia_admin_vencida') != ""){
                foreach($this->session->userdata('licencia_admin_vencida') as $almacen_vencido){
                    if(array_key_exists($almacen_vencido, $data['almacenes'])){
                        unset($data['almacenes'][$almacen_vencido]);
                    }
                }
            }
    ...
    /*public function import_excel_nombre_codigo() {
        ...
        if (!empty($_FILES['archivo']['name'])) {
            ...
            if (!$this->upload->do_upload('archivo')) {
                ...
                foreach ($sheetData as $index => $value) {
                    ...
                    if ($index != 1) {
                        ...
                        if ($value['A'] != '' && $value['C'] != "") {
                            ...
                            $id_producto = $this->inventario->add_csv_inventario_nombre_codigo($data, $array, $id);
                            $productoDetalle = $this->productos->codigoBarras(utf8_decode($value['A']));
                            if($productoDetalle != false)
                            {
                                $productos[] = array(                                
                                    ...
                                    "cantidad" => $value['C'],
                                    "precio_compra" => $productoDetalle->precio_compra,
                                    "existencias" => 0,
                                    "nombre" => $productoDetalle->nombre,
                                    "total_inventario" => (float) $value['C'] * (float) $productoDetalle->precio_compra,
                ...
                $this->inventario->add_csv_MovientosDetalles($productos);
    ...
    public function import_excel_nombre_codigo() {
        ...
        if (!empty($_FILES['archivo']['name'])) {
            ...
            if (!$this->upload->do_upload('archivo')) {
                ...
                $data['codigo_factura'] = isset($_POST['codigo_factura'])?$_POST['codigo_factura']:'';
                $id = $this->inventario->add_csv_inventario_1($data);
                ...
                foreach ($sheetData as $index => $value) {
                    ...
                    if ($index != 1) {
                        ...
                        if ($value['A'] != '' && $value['B'] != "") {
                            ...
                            $error_by_product = $this->inventario->add_csv_inventario_nombre_codigo($data, $array, $id);
                            $error_message .= $error_by_product;
                ...
                $result['valid'] = true;
                if($error_message != ''){//Se valida si todos los productos se importaron correctamente
                    $result['message_error'] = 'Algunos productos no han sido importados: '.$error_message;
                    $this->session->set_flashdata('message_error', custom_lang('sima_bill_send_message', '<p>Algunos productos no han sido importados:</p>'.$error_message));
                }else{
                    $result['message'] = 'Productos importados correctamente';
                    $this->session->set_flashdata('message', custom_lang('sima_bill_send_message', "Se ha guardado correctamente el inventario"));
                }
