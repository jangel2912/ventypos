...
class Ion_auth_model extends CI_Model
{
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$this->trigger_events('post_register');
				$this->db->where('id', $id);
			    $this->db->update('users',  array('db_config_id' => '1'));

		return (isset($id)) ? $id : FALSE;
	}

        public function validar($identity, $password)
        {
            ...
            $query = $this->db->select($this->identity_column . ', username, email, users.id, password, active, last_login, usuario, clave, servidor, base_dato, db_config_id, idioma, rol_id, is_admin')
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, users.id, password, active, last_login, usuario, clave, servidor, base_dato, db_config_id, idioma, rol_id, is_admin,db_config.estado,es_estacion_pedido')
		                  ->where($this->identity_column, $this->db->escape_str($identity))
                                  ->join('db_config', 'db_config.id = users.db_config_id', 'left')
		                  ->limit(1)
		                  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				{
					$this->trigger_events('post_login_unsuccessful');
					$this->set_error(' El inicio de sesi&oacute;n no est&aacute; activo para este usuario, por favor comunquese con el administrador del sistema. ');

					return FALSE;
				}
				//var_dump('luego de metodo');die();
	...
	**/
	...
	public function set_session($user)
	{
		...
		$session_data = array(
            ...
            'base_dato' 		   => $user->base_dato,
            'servidor' 			   => $user->servidor,
            'usuario' 			   => $user->usuario,
            'clave' 			   => $user->clave,
            'db_config_id' 		   => $user->db_config_id,
            'idioma' 			   => $user->idioma,
            'rol_id' 			   => $user->rol_id,
        ...
        if(!$this->ion_auth->in_group($group_licencias,$user->id)){
        	$dns = "mysql://{$user->usuario}:{$user->clave}@{$user->servidor}/{$user->base_dato}";
                $dbConnection = $this->load->database($dns, true);
                $query = "select * from permiso_rol where id_rol = {$user->rol_id}";
                $permisos = array();
                foreach ($dbConnection->query($query)->result() as $value) {
					if(($value->id_permiso!='1038')&&($user->is_admin!='t')){                  
						$permisos[] = $value->id_permiso;
					  }                    
                }
                $session_data['permisos'] = $permisos;
		        //sacamos el pais configurado 
		        $query = "select valor_opcion from opciones where nombre_opcion='pais'";
		        $id_pais = $dbConnection->query($query)->row();   
		        if($id_pais){
		        	$session_data['pais_idioma'] = $id_pais->valor_opcion; 	
		        }else{
		        	$session_data['pais_idioma'] = 1;
		        }     
        }        
