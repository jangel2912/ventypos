var VendtyApp = function(){
    ...
    return {
        ...
        // =========================================================================
        init: function () {
            VendtyApp.handleBaseURL();
            VendtyApp.inputMask();
            VendtyApp.handleTooltip();
            VendtyApp.handlePanelScroll();
            //VendtyApp.handleSidebarNavigation();
            //VendtyApp.handleSidebarResponsive();
            //VendtyApp.handleFullscreen();
            /*VendtyApp.handleIE();
            VendtyApp.handleFullscreen();
            /*VendtyApp.handleIE();
            VendtyApp.handleCheckCookie();
            VendtyApp.handleSound();
            VendtyApp.handleBackToTop();
            VendtyApp.handleSidebarNavigation();
            VendtyApp.handleSidebarScroll();
            VendtyApp.handleSidebarResponsive();
            VendtyApp.handleNavbarScroll();
            VendtyApp.handlePanelScroll();
            VendtyApp.handleTypehead();
            
            VendtyApp.handleSelect2();
            
            VendtyApp.handlePopover();
            VendtyApp.handlePanelToolAction();
            VendtyApp.handleSparkline();
            VendtyApp.handleClearCookie();*/
            VendtyApp.handleBoxModal();
            VendtyApp.handleCopyrightYear();
        },
        // =========================================================================
        // JQUERY INPUTMASK
        // =========================================================================
        ...
        handleSound: function () {
            if($('.page-sound').length){
                ion.sound({
                    ...
                    path: VendtyApp.handleBaseURL()+'/assets/global/plugins/bower_components/ionsound/sounds/',
                    preload: true
                });
        ...
        handleSidebarResponsive: function () {
            ...
            $(".navbar-minimize a").trigger("click");
        ...
        handleTypehead: function () {
            if($('.typeahead').length){
                ...
                repos = new Bloodhound({
                    ...
                    prefetch: VendtyApp.handleBaseURL()+'/assets/global/plugins/bower_components/typehead.js/dist/typeahead-sample.json'
                });
        ...
        handlePanelToolAction: function () {
            ...
            $('[data-action=refresh]').on('click', function(){
                ...
                setInterval(function(){
                    $.getJSON(VendtyApp.handleBaseURL()+'/assets/admin/data/reload-sample.json', function(json) {
                        $.each(json, function() {
        ...
        handleBoxModal: function () {
            $('#btn_new_producto').on('click', function(){
                // Add sound
                //ion.sound.play('camera_flashing');
                bootbox.dialog({
                    message: '<div class="panel panel-tab panel-tab-double shadow">'+
                                '<div class="panel-heading no-padding">'+
                                    ...
                                    '            <h4>Datos basicos</h4>'+
                                    '            <p>Datos Basicos y generales del producto</p>'+
                                    '        </div>'+
                                    '        <div class="tab-pane fade" id="tab6-2">'+
                                    '            <h4>Modificaciones</h4>'+
                                    '            <p>Modificaciones realizadas al producto</p>'+
                                    '        </div>'+
                                    '        <div class="tab-pane fade" id="tab6-3">'+
                                    '            <h4>Adicionales</h4>'+
                                    '            <p>Productos adicionales que modifican precio</p>'+
                                            '</div>'+
                                        '</div>'+
                                    '</div>'+
                                '</div>',
