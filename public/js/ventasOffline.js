...
function limpiarCampo(num) {
    if (num != '') {
        ...
        return num.toFixed(__decimales__);
    }
}
    ...
    $(".title-detalle").each(function (index, item) {
        ...
        if (porcentaje > 1) {
           ...
           // total += (total1 * cantidad);
            ...
            //total += redondear((redondear(precio + (precio * (impuesto / 100))) * cantidad) + vimpuesto);       
...
$(document).ready(function () {
    ...
    $("#search").keyup(function (e) {
        ...
        if (tipo_busqueda == 'buscalo') {
            ...
            appOffline.queryProductosBuscador( query ,function(){           

                var data = appOffline.getObjProductosBuscador();                
    ...
    function formatRows(row) {
        ...
        html += "<td><p><span class='nombre_producto'>" + row.nombre + "</span>&nbsp;<input type='hidden' value='" + row.precio_venta + "' class='precio-real'/><span class='precio'>" + precioimpuesto + "</span></p>";
        ...
        close: function () {
            ...
            if (cantidadsindescuentogeneral == 0) {
                ...
                if ($("#valor_entregado1").val() >= 0) {
                    $("#forma_pago1").val("efectivo").trigger("change");
                ...
                if ($("#valor_entregado2").val() >= 0) {
                    $("#forma_pago2").val("efectivo").trigger("change");
                ...
                if ($("#valor_entregado3").val() >= 0) {
                    $("#forma_pago3").val("efectivo").trigger("change");
                ...
                if ($("#valor_entregado4").val() >= 0) {
                    $("#forma_pago4").val("efectivo").trigger("change");
                ...
                if ($("#valor_entregado5").val() >= 0) {
                    $("#forma_pago5").val("efectivo").trigger("change");
    ...
    $("#grabar").click(function () {
        ...
        appOffline.guardarVenta( venta, function () {

                        

            var idVenta = appOffline.getIdVenta();

            window.idVenta = idVenta;

            var base = appOffline.getBase();         

            var plantilla = appOffline.getPlantilla();
            ...
            if (!confirm("Desea imprimir la factura de venta?")) {
                ...
                //var urlFactura = base+"offline/imprimir/"+plantilla+".html?id="+idVenta;

                urlFactura = base+"offline/imprimir/"+plantilla+".html";
...
$("#datos_cliente").autocomplete({    
    ...
    source:  function (request, response) { // Contains
        ...
        appOffline.queryClientes( query, function(){

            

            var clientes = appOffline.getObjClientes();                       
...
function codificaloAddValue() {
    ...
    var codigoBarras = $("#search").val();

 

    appOffline.queryProductosCodificalo( codigoBarras, function () {



        var data = appOffline.getObjProductosCodificalo();
...
function filtrarCategoria(element) {
    ...
    appOffline.queryProductosNavegador(function () {



        var response = appOffline.getObjProductosNavegador();
        ...
        for (var i = 0; i < response.length; i++) {
            ...
            $('#vitrina').append(
                    ...
                    '<img src="' + $urlImages + '/vendtyShop.jpg">' +
...
function siguiente_categorias() {
    ...
    $.ajax({
        ...
        if (response.length != 0) {
            ...
            html = '<li id="0" onclick="filtrarCategoria(this)"><img src="' + $urlImages + '/vendtyShop.jpg"><br>Todos</li>';

            html += '<li id="2" onclick="filtrarCategoria(this)"><img src="' + $urlImages + '/vendtyShop.jpg"><br>General</li>';

            for (var i = 0; i < (response.length); i++) {

                if (response[i].id != 2)

                    html += '<li id="' + response[i].id + '" onclick="filtrarCategoria(this)" ><img src="' + $urlImages + '/vendtyShop.jpg"><br>' + response[i].nombre + '</li>';
